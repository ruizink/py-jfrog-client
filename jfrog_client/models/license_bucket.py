...
class LicenseBucket(BaseModel):
    """
    LicenseBucket
    """ # noqa: E501
    identifier: Optional[StrictStr] = Field(default=None, description="Bucket identifier")
    name: Optional[StrictStr] = Field(default=None, description="Bucket name")
    size: Optional[StrictInt] = Field(default=None, description="Number of licenses in the bucket")
    license_type: Optional[StrictStr] = Field(default=None, description="License type")
    __properties: ClassVar[List[str]] = ["identifier", "name", "size", "license_type"]
    ...
    def license_type_validate_enum(cls, value):
        ...
        if value not in set(['ENTERPRISE', 'EDGE']):
            raise ValueError("must be one of enum values ('ENTERPRISE', 'EDGE')")
        return value
