...
class VaultConfig(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["key", "type", "config"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['HashicorpVault']):
            raise ValueError("must be one of enum values ('HashicorpVault')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "key": obj.get("key"),
            "type": obj.get("type"),
            "config": HashicorpVaultConfig.from_dict(obj["config"]) if obj.get("config") is not None else None
        })
        return _obj
