...
class FederationOperationResult(BaseModel):
    """
    Result entry from configure-federation operations (snake_case JSON)
    """ # noqa: E501
    label: Optional[StrictStr] = Field(default=None, description="Operation or step label")
    status: Optional[StrictStr] = Field(default=None, description="Result severity")
    status_message: Optional[StrictStr] = Field(default=None, description="Detail message when status is WARNING or ERROR")
    __properties: ClassVar[List[str]] = ["label", "status", "status_message"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['OK', 'WARNING', 'ERROR']):
            raise ValueError("must be one of enum values ('OK', 'WARNING', 'ERROR')")
        return value
