...
class HashicorpVaultAppRoleAuth(BaseModel):
    """
    HashicorpVaultAppRoleAuth
    """ # noqa: E501
    type: StrictStr
    role_id: Optional[StrictStr] = Field(default=None, description="AppRole role ID.", alias="roleId")
    secret_id: Optional[StrictStr] = Field(default=None, description="AppRole secret ID.", alias="secretId")
    __properties: ClassVar[List[str]] = ["type", "roleId", "secretId"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AppRole']):
            raise ValueError("must be one of enum values ('AppRole')")
        return value
