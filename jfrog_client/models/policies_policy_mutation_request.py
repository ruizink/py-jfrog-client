...
class PoliciesPolicyMutationRequest(BaseModel):
    """
    ...
    """ # noqa: E501
    name: StrictStr = Field(description="Name of the policy. Must be unique across the system (or within a project when using projectKey). ")
    description: Optional[StrictStr] = Field(default=None, description="Optional free-text description of the policy.")
    type: StrictStr = Field(description="The policy type. Determines which criteria fields are available in the rules. ")
    rules: List[PolicyRule] = Field(description="One or more rules that define the policy behavior. Each rule specifies criteria to match and actions to take when matched. ")
    __properties: ClassVar[List[str]] = ["name", "description", "type", "rules"]
