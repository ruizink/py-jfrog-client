# coding: utf-8

"""
    JFrog Platform API (Unofficial)

    **Unofficial** combined REST API spec for the JFrog Platform. This is not an official JFrog specification and may be incomplete or inaccurate.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from jfrog_client.models.replication_request import ReplicationRequest

class TestReplicationRequest(unittest.TestCase):
    """ReplicationRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ReplicationRequest:
        """Test ReplicationRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ReplicationRequest`
        """
        model = ReplicationRequest()
        if include_optional:
            return ReplicationRequest(
                url = '',
                username = '',
                password = '',
                proxy = '',
                disable_proxy = True,
                properties = True,
                delete = True,
                include_path_prefix_pattern = '',
                exclude_path_prefix_pattern = '',
                check_binary_existence_in_filestore = True
            )
        else:
            return ReplicationRequest(
        )
        """

    def testReplicationRequest(self):
        """Test ReplicationRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
